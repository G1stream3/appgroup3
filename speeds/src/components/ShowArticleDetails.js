...
class ShowArticleDetails extends Component {
  ...
  componentDidMount() {
    axios
      .get('https://speedappgroup3.herokuapp.com//api/articles/article/'+this.props.match.params.id) //get the details of a specific article
      .then(res => {
        this.setState({
          article: res.data
        })
  ...
  onDeleteClick (id) {
    axios
      .delete('https://speedappgroup3.herokuapp.com//api/articles/'+id) //delete an article from the database (moderater declined article)
      .then(res => {
  ...
  onAccept = e => {
    ...
    axios
      .put('https://https://speedappgroup3.herokuapp.com//api/articles/'+this.props.match.params.id, data) //return to moderation page after approving
      .then(res => {
        this.props.history.push('/moderation');
      })
      .catch(err => {
